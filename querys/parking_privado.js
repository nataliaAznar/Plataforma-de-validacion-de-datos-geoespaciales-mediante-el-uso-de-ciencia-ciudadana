...
exports.getSolution = function getSolution(idError, callback){
  client.connect(function(err) {
	...
	client.query( "SELECT problem, COUNT(*)  FROM validations WHERE Error_type = 114 group by problem", function(err, result){
	  if(err){
	    ...
	    if ( problem == "" ){
		client.query( "SELECT count(tags->'amenity') AS amenity, count(tags->'site') AS site, count(tags->'access') AS access, count(tags->'service') AS service, count(tags->'building') AS building, count(tags->'landuse') AS landuse FROM validations WHERE error_type = 114  ", function (err, result){ 
		    if(err){
		      console.log("error getting solution of error114 "+err);
		      client.end();
		    }
		      ...
		      client.query( "SELECT (tags->'"+name+"') AS name, count(tags->'"+name+"') AS count FROM validations WHERE error_type = 114 and (tags->'"+name+"') is not null GROUP BY (tags->'"+name+"')", function (err, result){
			  if(err){
			    console.log("error getting solution of error114 "+err);
			    client.end();
			  }
	      ...
	      client.query( "SELECT GeometryType(geom) as type, * FROM error_114 WHERE idError = "+idError+";", function (err, result){
		  if(err){
		    console.log("error getting solution of error114 "+err);
		    client.end();
		  }
