	    ...
	    if(!result.rows[0].exists){
		  ...
		  WITH (\
			...
			if ( err ) {
			  ...
			  client.query( "INSERT INTO error VALUES("+numError+", '"+errorDesc+"', '"+title+"', '"+tableName+"', 'parking_privado.js');" , function(err, result){
  ...
  client.connect(function(err) {
	...
	client.query( "SELECT problem, COUNT(*)  FROM validations WHERE error_type = 114 AND error_id = " + idError + " GROUP BY problem ORDER BY count desc, problem desc", function(err, result){
	  if(err){
	    ...
	    if ( problem == "" ){
		client.query( "SELECT count(tags[1]->'amenity') AS amenity, count(tags[1]->'site') AS site, count(tags[1]->'access') AS access, count(tags[1]->'service') AS service, count(tags[1]->'building') AS building, count(tags[1]->'landuse') AS landuse FROM validations WHERE error_type = 114 AND error_id = " + idError + "; ", function (err, result){ 
		    if(err){
		      console.log("error getting solution of error114 "+err);
		      client.end();
		      callback();
		      ...
		      client.query( "SELECT (tags[1]->'"+name+"') AS name, count(tags[1]->'"+name+"') AS count FROM validations WHERE error_type = 114 AND error_id = " + idError + " AND (tags[1]->'"+name+"') is not null GROUP BY (tags[1]->'"+name+"') ORDER BY count", function (err, result){
			  if(err){
			    console.log("error getting solution of error114 "+err);
			    client.end();
			    callback();
